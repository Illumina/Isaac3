...
class Cigar: public std::vector<uint32_t, common::NumaAllocator<uint32_t, common::numa::defaultNodeLocal> >
{
    typedef std::vector<uint32_t, common::NumaAllocator<uint32_t, common::numa::defaultNodeLocal> > BaseT;
public:
    Cigar (const std::size_t n, const uint32_t &init) : BaseT(n, init)
    {
    }

    Cigar(const BaseT &that) : BaseT(that)
    {
    }
    ...
    void addOperation(int64_t length, OpCode opCode)
    {
        uint64_t ulength = std::abs(length);
    ...
    void updateOperation(const unsigned offset, uint64_t len, const Cigar::OpCode op)
    {
    ...
    struct Encoder
    {
        ...
        Encoder(uint64_t length, OpCode opCode):
...
private:
    static uint32_t encode(uint64_t &length, OpCode opCode)
    {
