...
struct Anchor : public std::pair<unsigned short, unsigned short>
{
    ...
    {
        return os << "Anchor(" << a.first << "," << a.second << "," << (a.kUnique_ ? "ku," : "") << (a.kRepeat_ ? "kr" : "") << ")";
    }
};
...
struct FragmentMetadata
{
    ...
    FragmentMetadata(const Cluster *cluster, unsigned readIndex,
                     ...
                     const unsigned contigId = reference::ReferencePosition::MAX_CONTIG_ID, const int64_t position = 0):
        cluster(cluster), contigId(contigId), position(position), lowClipped(lowClipped), highClipped(highClipped), adapterClipped_(0),
...
/// First cycle of fragment bcl data
    ...
    int64_t getEndClippedLength() const
    {
        if (cigarBuffer && cigarLength)
        {
    ...
    bool startAnchored() const {return firstAnchor_.good();}
    bool endAnchored() const {return lastAnchor_.good();}
        ...
        const SeedId &seedId)
    {
        ...
        if (reverse)
        {
            ...
            firstAnchor_.first = readMetadata.getLength() - seedMetadata.getOffset() - seedId.getSeedLength();
        }
        else
        {
            firstAnchor_.first = seedMetadata.getOffset();
        }
        firstAnchor_.second = firstAnchor_.first + seedId.getSeedLength();
    ...
    /*
     * \brief make sure best anchors are kept in the fragment.
     *        Best anchors are those that are most further apart and shorter in length
     */
    void mergeAnchors(const FragmentMetadata &that)
    {
        highRepeat &= that.highRepeat;
        // endAnchored if startAnchored
        if (!that.firstAnchor_.empty())
        {
            if (that.firstAnchor_.betterThan(firstAnchor_) ||
                (!firstAnchor_.betterThan(that.firstAnchor_) &&
                    (firstAnchor_.second > that.firstAnchor_.second ||
                        (firstAnchor_.second == that.firstAnchor_.second && firstAnchor_.first < that.firstAnchor_.first))))
            {
                firstAnchor_ = that.firstAnchor_;
            }

            if (!that.lastAnchor_.empty() &&
                (that.lastAnchor_.betterThan(lastAnchor_) ||
                    (!lastAnchor_.betterThan(that.lastAnchor_) &&
                        (lastAnchor_.first < that.lastAnchor_.first ||
                            (lastAnchor_.first == that.lastAnchor_.first && lastAnchor_.second > that.lastAnchor_.second)))))
            {
                lastAnchor_ = that.lastAnchor_;
            }
        }
        else
        {
            // otherwise new anchors are empty, so, nothing to merge.
            ISAAC_ASSERT_MSG(that.lastAnchor_.empty(), "Invalid combination of anchors " << *this << " that:" << that);
        }
...
BOOST_STATIC_ASSERT(sizeof(FragmentMetadata) <= 240);
#endif
