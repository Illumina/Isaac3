...
public:
    ...
    int64_t mateMinPosition(unsigned readIndex, bool reverse, int64_t position, const unsigned *readLengths) const;
    /// Determine the maximal mate position for the given fragment
    int64_t mateMaxPosition(unsigned readIndex, bool reverse, int64_t position, const unsigned *readLengths) const;
    /**
     ...
     ** forward and reverse) of the fragments and on their relative position
     ** ('p' if first cycle position of f1 <= first cycle position of f2, 'm' otherwise):
     **
     ** - 0: FFp -> Fp
     ** - 1: FRp -> Rp
     ** - 2: RFp -> Rm
    ...
    template <typename FragmenT> static AlignmentModel alignmentModel(const FragmenT &f1, const FragmenT &f2)
    {
        if (f1.getContigId() == f2.getContigId())
        {
            const unsigned positionMask = (f1.getStrandReferencePosition() <= f2.getStrandReferencePosition()) ? 0 : 4;
    ...
    /**
     * \brief Attempt to calculate the length of the sequenced fragment from the pair of read alignments. Notice
     *        that strictly speaking this can only be done for standard paired-end template. For mate-pair templates,
     *        the distance between the last sequenced cycles of both reads is taken, however this is only close to
     *        reality when the length of mate pair is much shorter than the length of circularized fragment.
     */
    template <typename FragmenT> static uint64_t getLength(const FragmenT &f1, const FragmenT &f2)
    {
        ISAAC_ASSERT_MSG(f1.getContigId() == f2.getContigId(), "Both fragments must align to the same contig");
        if(f1.isReverse() != f2.isReverse())
        {
            // We need to handle a special case of standard template being shorter than read length.
            if (!f1.isReverse())
            {
                if (f1.getFStrandReferencePosition() > f2.getRStrandReferencePosition())
                {
                    return f1.getRStrandReferencePosition() - f2.getFStrandReferencePosition() + 1;
                }
                else
                {
                    return f2.getRStrandReferencePosition() - f1.getFStrandReferencePosition() + 1;
                }
            }
            else
            {
                return getLength(f2,f1);
            }
        }
        else
        {
            // Just take the maximum length of the possible relative positions of the fragments.
            // Ignoring a possibility of one read to be totally included in the other (longer) read
            return std::max(
                std::abs(f1.getFStrandReferencePosition() - f2.getRStrandReferencePosition()),
                std::abs(f1.getRStrandReferencePosition() - f2.getFStrandReferencePosition()));
        }
    }
...
public:
     ...
     **/
    bool appendTemplate(const FragmentMetadataList &r0Fragments, const FragmentMetadataList &r1Fragments);
    bool appendUniqueTemplate(const FragmentMetadata &perfect0, const FragmentMetadata &perfect1);
    void appendModel(const TemplateLengthStatistics::AlignmentModel am, const uint64_t length);
    ...
    typedef std::pair<TemplateLengthStatistics::AlignmentModel, uint64_t> AlignmentModel;
    static AlignmentModel getAlignmentModel(const FragmentMetadataList &r0Fragments, const FragmentMetadataList &r1Fragments);
...
private:
    ...
    static const double STANDARD_DEVIATIONS_MAX;
    ...
    {
        std::size_t samples_;
        typedef boost::array<std::size_t, TemplateLengthStatistics::TEMPLATE_LENGTH_THRESHOLD> Bars;
        Bars bars_;
    public:
        ...
        bool append(unsigned insertSize)
        {
            if (TemplateLengthStatistics::TEMPLATE_LENGTH_THRESHOLD > insertSize)
            {
                ++bars_[insertSize]; ++samples_;
                return true;
            }
            return false;
        }
        std::size_t samples() const {return samples_;}
        void clear() {std::fill(bars_.begin(), bars_.end(), 0); samples_ = 0;}
        Histogram &operator +=(const Histogram &that)
        {
            std::transform(bars_.begin(), bars_.end(), that.bars_.begin(), bars_.begin(),
                           [](const Bars::value_type left, const Bars::value_type right) noexcept {return left + right;});
            samples_ += that.samples_;
            return *this;
        }
        Bars::value_type operator [](const std::size_t i) const {return bars_[i];}
    };

    boost::array<Histogram, TemplateLengthStatistics::InvalidAlignmentModel> histograms_;

    void updateStatistics();
    static AlignmentModel getAlignmentModel(const FragmentMetadata &perfect0, const FragmentMetadata &perfect1);

#ifndef _GLIBCXX_DEBUG
// When changing this size, make sure it is still reasonalbe enough to be put on stack
BOOST_STATIC_ASSERT(sizeof(TemplateLengthDistribution::Histogram) == 400008);
#endif
};
