...
public:
        ...
        : distributionBinSize_(matchDistribution.getBinSize())
    {
        ...
        BOOST_FOREACH(const std::vector<unsigned> &contigDistribution, matchDistribution)
        {
            ...
            uint64_t currentBinMatches = 0;
            uint64_t currentContigMatches = 0;
            BOOST_FOREACH(unsigned count, contigDistribution)
            {
                ...
                // if count already exceeds outputBinMatches
                if (currentBinMatches && currentBinMatches + count > matchesPerBin)
                {
                    ++currentBinIndex;
                    currentBinMatches = 0;
                }
                currentBinMatches += count;
                currentContigMatches += count;
                back().push_back(currentBinIndex);
            }
            // bins do not spread across contigs
            if (!skipEmptyBins || currentContigMatches)
            {
    ...
    std::size_t getBinIndex(const isaac::reference::ReferencePosition &referencePosition) const
    {
        ...
        const uint64_t index = position / distributionBinSize_;
    ...
    isaac::reference::ReferencePosition getBinFirstPos(const unsigned bin) const
    {
        ...
        const uint64_t position = distributionBinSize_ * (contigBinIterator - binContigIterator->begin());
    ...
    isaac::reference::ReferencePosition getBinFirstInvalidPos(const unsigned bin) const
    {
        ...
        const uint64_t contigId = binContigIterator - begin() - 1;
        const uint64_t position = distributionBinSize_ * (contigNextBinIterator - binContigIterator->begin());
