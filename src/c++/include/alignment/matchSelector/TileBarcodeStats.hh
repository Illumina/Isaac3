...
struct TileBarcodeStats
{
    ...
    uint64_t yield_;
    uint64_t yieldQ30_;
    uint64_t qualityScoreSum_;
    uint64_t clusterCount_;
    uint64_t unanchoredClusterCount_;
    uint64_t nmnmClusterCount_;
    uint64_t rmClusterCount_;
    uint64_t qcClusterCount_;
    ...
    uint64_t uniquelyAlignedPerfectFragmentCount_;
    uint64_t alignmentScoreSum_;
    uint64_t basesOutsideIndels_;
    uint64_t uniquelyAlignedBasesOutsideIndels_;
    uint64_t mismatches_;
    uint64_t uniquelyAlignedMismatches_;
    uint64_t alignmentModelCounts_[TemplateLengthStatistics::InvalidAlignmentModel+1];
    uint64_t nominalModelCounts_[TemplateLengthStatistics::CheckModelLast];
    ...
    {
        ISAAC_ASSERT_MSG(readMetadata.getLength() == fragment.getYield(), "Expected read length to match the read yield " << readMetadata << " " << fragment);
