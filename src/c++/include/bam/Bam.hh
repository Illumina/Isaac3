    ...
    const THeader &header)
{
    ...
    BOOST_FOREACH(const typename THeader::ReadGroupType &readGroup, header.getReadGroups(bamPuFormat))
    {
...
unsigned serializeAlignment(std::ostream &os, T&alignment)
{
    ...
    const typename T::SeqBeginEnd seqBeginEnd = alignment.seq();
    const size_t seqLength = std::distance(seqBeginEnd.first, seqBeginEnd.second);

    const typename T::QualBeginEnd qualBeginEnd = alignment.qual();
    const size_t qualLength = std::distance(qualBeginEnd.first, qualBeginEnd.second);
    ...
    const zTag fragmentSA = alignment.getFragmentSA();
    ...
    serialize(os, seqBeginEnd);  //1
    serialize(os, qualBeginEnd); //2
    ...
    if (!fragmentSA.empty()) serialize(os, fragmentSA);
