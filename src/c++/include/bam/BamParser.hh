...
InsertIt extractReadName(const BamBlockHeader &bamBlock, const unsigned nameLengthMax, InsertIt insertIt)
{
    ...
    char szBuffer[200];
    std::size_t len = 0;
    if (bamBlock.isReadOne())
    {
        len = snprintf(szBuffer, sizeof(szBuffer)-1, "%c:%02d:%010d", bamBlock.isUnmapped() ? 'u' : bamBlock.isReverse() ? 'r':'f', bamBlock.getRefId(), bamBlock.getPos());
    }
    else
    {
        len = snprintf(szBuffer, sizeof(szBuffer)-1, "%c:%02d:%010d", bamBlock.isNextUnmapped() ? 'u' : bamBlock.isNextReverse() ? 'r':'f', bamBlock.getNextRefId(), bamBlock.getNextPos());
    }
    *(szBuffer + len) = '-';
    ++len;
    ...
    if (!bamBlock.isReadOne())
    {
        len += snprintf(szBuffer + len, sizeof(szBuffer) - len, "%c:%02d:%010d", bamBlock.isUnmapped() ? 'u' : bamBlock.isReverse() ? 'r':'f', bamBlock.getRefId(), bamBlock.getPos());
    }
    else
    {
        len += snprintf(szBuffer + len,  sizeof(szBuffer) - len, "%c:%02d:%010d", bamBlock.isNextUnmapped() ? 'u' : bamBlock.isNextReverse() ? 'r':'f', bamBlock.getNextRefId(), bamBlock.getNextPos());
    }

    return extractReadName(szBuffer, len, nameLengthMax, insertIt);
#else
