...
int sprintFloat(char *buffer, const float originalValue)
{
    ...
    assert(std::numeric_limits<int64_t>::min() < floatValue
            && std::numeric_limits<int64_t>::max() > floatValue);
    const float rounding = (originalValue < -0.0 ? -0.5f : 0.5f);
    int64_t value = static_cast<int64_t> (floatValue + rounding);
...
{
    const std::size_t initialLength = s.size();
    NumberT u = value;
...
template <typename ContainerT, typename NumberT>
inline  void appendInteger(ContainerT &s, NumberT value)
{
    if (0 > value)
    {
        s.push_back('-');
 ...
 ** int64_t, and generally on any type supporting '<=', '%', '/=', '+'
 ...
 ** @brief Fast and portable output of an unsigned integer into a stream.
 **
 ** Can be instantiated on (signed versions of) char, short, int and int64_t.
 **
...
uint64_t getUnsignedInteger(IteratorT begin, IteratorT end)
{
    uint64_t ret = 0;
    while (end != begin)
    {
...
public:
    FastIoString(IteratorT begin, IteratorT end) : begin_(begin), end_(end){}
    friend std::ostream &operator <<(std::ostream &os, const FastIoString &s)
    {
        BOOST_FOREACH(const char c, std::make_pair(s.begin_, s.end_))
        {
            os << c;
        }
        return os;
...
template <typename IteratorT>
FastIoString<IteratorT> makeFastIoString(IteratorT begin, IteratorT end)
{
    return FastIoString<IteratorT>(begin, end);
}
