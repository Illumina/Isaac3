...
extern __thread int runOnNode_;
...
public:
     ...
     */
    BasicThreadVector(size_type size, int targetNumaNode = numa::defaultNodeInterleave) : executor_(0), busyThreads_(size), neededThreads_(0),
        lowestBlockedThreadNumber_(0), terminateRequested_(false), currentRequest_(0),
        targetNumaNode_(common::numa::defaultNodeLocal == targetNumaNode ?
            getThreadNumaNode() : targetNumaNode)
    {
//        getitimer(ITIMER_PROF, &itimer_);
    ...
    static int getThreadNumaNode() {return runOnNode_;}
//    static int getThreadNumaNode(const unsigned threadNumber) {return common::getThreadNumaNode(threadNumber);}
...
private:
    ...
    void threadFunc(const size_type threadNum)
    {
        runOnNode_ = common::bindCurrentThreadToNumaNode(
            common::numa::defaultNodeInterleave == targetNumaNode_ ?
                common::getThreadInterleaveNumaNode(threadNum) : targetNumaNode_);
