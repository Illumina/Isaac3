...
inline std::ostream & operator << (std::ostream &os, const BclFlowcellData &fd)
{
    return os << "BclFlowcellData(" <<
        fd.softwareVersion_.first << " " << fd.softwareVersion_.second << "sv," <<
        fd.softwareMajorMinor_.first << " " << fd.softwareMajorMinor_.second << "smm," <<
        fd.compressed_ << "c," <<
        fd.patternedFlowcell_ << "pf," <<
        fd.tilesPerLaneMax_ << "tlm)";
}
...
inline unsigned getMaxCycleNumber(const flowcell::Layout &flowcell)
{
    unsigned ret = getMaxCycleNumber(flowcell.getReadMetadataList());
...
inline unsigned getMaxCycleNumber(const FlowcellLayoutList &flowcellLayoutList)
{
    ...
    BOOST_FOREACH(const flowcell::Layout &flowcell, flowcellLayoutList)
    {
        ret = std::max(ret, getMaxCycleNumber(flowcell));
    }
    return ret;
}
