...
typedef std::vector<ReadMetadata> ReadMetadataList;
...
inline unsigned getMinReadLength(const flowcell::ReadMetadataList &readMetadataList)
{
    return std::min_element(readMetadataList.begin(), readMetadataList.end(),
                            boost::bind(&flowcell::ReadMetadata::getLength, _1)<
                            boost::bind(&flowcell::ReadMetadata::getLength, _2))->getLength();
}
