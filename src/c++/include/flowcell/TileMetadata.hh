...
public:
    ...
    void setClusterCount(const unsigned clusterCount) {clusterCount_ = clusterCount;}
...
//    TileMetadataList(size_t size) : std::vector<flowcell::TileMetadata>(size){}
    TileMetadataList(const std::vector<flowcell::TileMetadata> &that) : std::vector<flowcell::TileMetadata>(that){}
};
...
inline std::ostream &operator<<(std::ostream &os, const TileMetadata &tileMetadata)
{
    return os << "TileMetadata(" 
              ...
              << tileMetadata.getOriginalIndex() << "oi, "
