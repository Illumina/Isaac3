...
//            ISAAC_THREAD_CERR << "positionStr:" << positionStr << std::endl;
            uint64_t pos = boost::lexical_cast<uint64_t>(positionStr) - 1; // wig is 1-based
            const std::string valueStr = line.substr(positionStr.length());
            int64_t value = boost::lexical_cast<int64_t>(valueStr.substr(valueStr.find_first_not_of(" \t")));
