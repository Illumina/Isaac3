...
public:
            ...
            const std::vector<unsigned> &contigTranslationTable) const
    {
        ...
        if (contigValue)
        {
            const uint64_t translatedContig = contigTranslationTable.at(contigValue - 1) + 1;
    ...
    uint64_t getPosition() const {return (value_ & POSITION_MASK) >> (REVERSE_BITS + NEIGHBORS_BITS);}
...
//    // it is not unusual for a metagenomics reference to have 16M contigs.
    static const uint64_t CONTIG_ID_BITS = 25;
    static const uint64_t POSITION_BITS = 37;
    static const uint64_t REVERSE_BITS = 1;
    static const uint64_t NEIGHBORS_BITS = 1;
    static const uint64_t MAX_CONTIG_ID = ((~static_cast<value_type>(0)) >> (POSITION_BITS + REVERSE_BITS + NEIGHBORS_BITS));
    static const uint64_t POSITION_MASK = (((~static_cast<value_type>(0)) >> (CONTIG_ID_BITS + REVERSE_BITS + NEIGHBORS_BITS)) << (REVERSE_BITS + NEIGHBORS_BITS));
    static const uint64_t POSITION_REVERSE_NEIGBORS_MASK = ((~static_cast<value_type>(0)) >> CONTIG_ID_BITS);
    static const uint64_t REVERSE_MASK = ((~static_cast<value_type>(0)) >> (CONTIG_ID_BITS + POSITION_BITS + NEIGHBORS_BITS));
    static const uint64_t NEIGHBORS_MASK = ((~static_cast<value_type>(0)) >> (CONTIG_ID_BITS + POSITION_BITS + REVERSE_BITS)) << REVERSE_BITS;
private:
    value_type value_;
