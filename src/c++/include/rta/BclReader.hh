...
public:
    ...
    {
        reserveBuffers(that.cycleFilePath_.string().capacity());
    }

    BclReader(const std::size_t reservePathLength, const bool ignoreMissingBcls, const unsigned maxClusters):
        ignoreMissingBcls_(ignoreMissingBcls),
        decompressor_(maxClusters),
        bclFileBuffer_(1, std::ios_base::in | std::ios_base::binary)
    {
        reserveBuffers(reservePathLength);
    }
...
private:
                           ...
                           const std::size_t bufferSize)
    {
        try
        {
            ...
            ISAAC_VERIFY_MSG(4 <= decompressedBytes,
