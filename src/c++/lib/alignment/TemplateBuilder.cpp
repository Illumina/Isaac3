...
TemplateBuilder::TemplateBuilder(
    ...
    , alignmentsMax_((repeatThreshold * maxSeedsPerRead) * (repeatThreshold * maxSeedsPerRead))
    ...
    , fragmentBuilder_(collectMismatchCycles, flowcellLayoutList, repeatThreshold, maxSeedsPerRead, gappedMismatchesMax, smitWatermanGapsMax,
    ...
    , shadowAligner_(collectMismatchCycles, flowcellLayoutList,
                     gappedMismatchesMax, smitWatermanGapsMax, smartSmithWaterman, noSmithWaterman, alignmentCfg_, reserveBuffers)
    , shadowList_()//(templateBuilder::TRACKED_REPEATS_MAX_ONE_READ)
    , bestCombinationPairInfo_(0,0)//(repeatThreshold, maxSeedsPerRead)
    , bestRescuedPair_(0,0)//(repeatThreshold, maxSeedsPerRead)

{
    if (reserveBuffers)
    {
        cigarBuffer_.reserve(Cigar::getMaxOperationsForReads(flowcellLayoutList) * alignmentsMax_);
        shadowList_.reserve(templateBuilder::TRACKED_REPEATS_MAX_ONE_READ);
        bestCombinationPairInfo_.reserve(repeatThreshold, maxSeedsPerRead);
        bestRescuedPair_.reserve(repeatThreshold, maxSeedsPerRead);
        std::for_each(fragments_.begin(), fragments_.end(), boost::bind(&FragmentMetadataList::reserve, _1, alignmentsMax_));
    }
    ...
    const unsigned mapqThreshold)
{
    bool ret = buildTemplate(
        contigList, kUniqeness, restOfGenomeCorrection, readMetadataList, sequencingAdapters, fragments_, cluster, templateLengthStatistics);
...
FragmentMetadataList::const_iterator TemplateBuilder::getBestFragment(const FragmentMetadataList &fragmentList) const
{
    common::StaticVector<FragmentMetadataList::const_iterator, templateBuilder::TRACKED_REPEATS_MAX_ONE_READ> bestFragments;
    ...
    const bool forceWellAnchored) const
{
    ...
    if (forceWellAnchored || fragment.isWellAnchored())
    {
        ...
        for (FragmentMetadataList::const_iterator i = fragmentList.begin(); fragmentList.end() != i; ++i)
        {
...
FragmentMetadata TemplateBuilder::cloneWithCigar(const FragmentMetadata &right)
{
    ...
    cigarBuffer_.reserve(cigarBuffer_.size() + std::distance(right.cigarBegin(), right.cigarEnd()));
    cigarBuffer_.addOperations(right.cigarBegin(), right.cigarEnd());
    return ret;
}

FragmentMetadata TemplateBuilder::trimForwardPEAdapter(
    const reference::ContigList &contigList,
    const isaac::reference::ContigAnnotations &kUniqenessAnnotation,
...
FragmentMetadata TemplateBuilder::trimReversePEAdapter(
    const reference::ContigList &contigList,
    const isaac::reference::ContigAnnotations &kUniqenessAnnotation,
    const flowcell::ReadMetadata &readMetadata,
    ...
    const reference::ReferencePosition &adapterPosition)
{
    ...
    const std::size_t currentOffset = std::distance(reverseFragment.cigarBegin(), current + 1);
    // ensure reallocation does not occur during the copying.
    ...
    cigarBuffer_.addOperations(reverseFragment.cigarBegin() + currentOffset, reverseFragment.cigarEnd());
...
std::pair<FragmentMetadata &, FragmentMetadata &> TemplateBuilder::trimPEAdapter(
    const reference::ContigList &contigList,
    const isaac::reference::ContigAnnotations &kUniqenessAnnotation,
    const flowcell::ReadMetadataList &readMetadataList,
    const FragmentMetadata & forwardFragment,
    const FragmentMetadata & reverseFragment)
{
    ...
    {
        trimmedAlignments_.push_back(trimForwardPEAdapter(
            contigList, kUniqenessAnnotation, readMetadataList[forwardFragment.getReadIndex()], forwardFragment, reverseFragment.getRStrandReferencePosition()));
    ...
    {
        trimmedAlignments_.push_back(trimReversePEAdapter(
            contigList, kUniqenessAnnotation, readMetadataList[reverseFragment.getReadIndex()], reverseFragment, forwardFragment.getFStrandReferencePosition()));
...
std::pair<FragmentMetadata &, FragmentMetadata &> TemplateBuilder::checkTrimPEAdapter(
    const reference::ContigList &contigList,
    const isaac::reference::ContigAnnotations &kUniqenessAnnotation,
    const flowcell::ReadMetadataList &readMetadataList,
    ...
    if (trimPEAdapters_)
    {
        if (r1Fragment.getContigId() == r1Fragment.getContigId())
        {
            if (r1Fragment.isReverse() != r2Fragment.isReverse())
            {
                ...
                {
                    return trimPEAdapter(contigList, kUniqenessAnnotation, readMetadataList, r1Fragment, r2Fragment);
                }
                else
                {
                    std::pair<FragmentMetadata &, FragmentMetadata &> ret =
                        trimPEAdapter(contigList, kUniqenessAnnotation, readMetadataList, r2Fragment, r1Fragment);
    ...
    templateBuilder::BestPairInfo &ret)
{
    ...
    for(FragmentIterator r1Fragment = fragments[0].begin(); fragments[0].end() != r1Fragment; ++r1Fragment)
    {
        ...
        {
            const std::pair<FragmentMetadata &, FragmentMetadata &> updatedAlignments =
                checkTrimPEAdapter(contigList, kUniqenessAnnotation, readMetadataList, *r1Fragment, *r2Fragment);
            const templateBuilder::PairInfo pairInfo(
    ...
    templateBuilder::BestPairInfo &ret)
{
        ...
        fragments[orphanIndex].end() != orphanIterator; ++orphanIterator)
    {
        ...
        ISAAC_THREAD_CERR_DEV_TRACE_CLUSTER_ID(orphan.getCluster().getId(), "TemplateBuilder::rescueShadow Orphan: " << orphan);
        ...
        {
            ISAAC_THREAD_CERR_DEV_TRACE_CLUSTER_ID(orphan.getCluster().getId(), "TemplateBuilder::rescueShadow orphan too bad to try rescuing shadows");
        }
        else if (shadowAligner_.rescueShadow(
            contigList, kUniqenessAnnotation, orphan, shadowList_, readMetadataList[shadowIndex], sequencingAdapters,
            templateLengthStatistics))
        {
            ...
            else
            {
                ...
                const std::pair<FragmentMetadata &, FragmentMetadata &> updatedAlignments =
                    checkTrimPEAdapter(contigList, kUniqenessAnnotation, readMetadataList, *orphanIterator, *shadowList_.begin());
...
bool TemplateBuilder::checkUpdateBest(
    ...
    templateBuilder::BestPairInfo& ret)
{
    ...
    else if (knownBestPair.empty() || (knownBestPair.bestPairMismatchCount() + SKIP_ORPHAN_HAMMING_DISTANCE) >= rescuedMismatches)
    {
        std::pair<FragmentMetadata &, FragmentMetadata&> updatedAlignments =
            checkTrimPEAdapter(contigList, kUniqenessAnnotation, readMetadataList, orphan, bestRescued);
        ...
        if (1 == knownBestPair.repeatsCount())
        {
            ...
            const FragmentMetadata &knownShadowRead = knownBestPair.repeat(0).getFragmentMetadata(
                updatedAlignments.second.getReadIndex());
    ...
    else
    {
        ISAAC_THREAD_CERR_DEV_TRACE_CLUSTER_ID(orphan.getCluster().getId(),
            "rescueDisjointedTemplate: ignoring template: " << orphan << "-" << bestRescued << " for mismatches " << rescuedMismatches << ">" << (knownBestPair.bestPairMismatchCount() + SKIP_ORPHAN_HAMMING_DISTANCE));
    }

    return false;
}

void TemplateBuilder::rescueDisjointedTemplate(
    ...
    templateBuilder::BestPairInfo& ret)
{
    for (FragmentIterator orphanIterator = orphansBegin; orphansEnd != orphanIterator; ++orphanIterator)
    {
        const FragmentMetadata &orphan = *orphanIterator;
        if (orphan.isHighRepeat())
        {
        ...
        const bool skipThisOrphan = (!knownBestPair.empty() ? //when ed cutoff is set, no point in trying orphans with greater edit distance
            ...
            ISAAC_LP_LESS(orphan.logProbability + ORPHAN_LOG_PROBABILITY_SLACK_, bestDisjointedFragments[orphan.getReadIndex()]->logProbability));
        ...
        if (!skipThisOrphan &&
            shadowAligner_.rescueShadow(contigList, kUniqenessAnnotation, orphan, shadowList_,
                                        readMetadataList[(orphan.getReadIndex() + 1) % READS_MAX], sequencingAdapters, templateLengthStatistics))
        {
            skipOneShadow = checkUpdateBest(orphan, shadowList_.front(), knownBestPair, contigList,
                                            kUniqenessAnnotation, readMetadataList, ret);
        }
        else if (!shadowList_.empty())
        {
            // The shadow hits a repetitive region next to one of the orphans
    ...
    templateBuilder::BestPairInfo &ret)
{
    ...
    ret.clear();
    rescueDisjointedTemplate(
        fragments[0].begin(), fragments[0].end(), knownBestPair, bestDisjointedFragments, contigList,
        kUniqenessAnnotation, readMetadataList, sequencingAdapters, templateLengthStatistics, ret);
    rescueDisjointedTemplate(
        fragments[1].begin(), fragments[1].end(), knownBestPair, bestDisjointedFragments, contigList,
        kUniqenessAnnotation, readMetadataList, sequencingAdapters, templateLengthStatistics, ret);
