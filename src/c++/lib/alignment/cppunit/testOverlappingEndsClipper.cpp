...
static const isaac::reference::Contig makeContig(const std::string forward, int64_t &firstPosOffset)
{
    std::string::const_iterator begin = std::find_if(forward.begin(), forward.end(),
                                                     [](char c){return c != ' ';});
//                                                     boost::bind(&boost::cref<char>, _1) != ' ');
    isaac::reference::Contig ret(0, "vasja", std::vector<char>(begin, forward.end()));
    ...
    isaac::reference::ContigList &contigList)
{
    ...
    templ.getFragmentMetadata(0).observedLength = readMetadataList.at(0).getLength() - overhang;
    ...
    templ.getFragmentMetadata(1).observedLength = readMetadataList.at(1).getLength() - overhang;
    if (overhang)
    {
        cigarBuffer_.addOperation(overhang, isaac::alignment::Cigar::SOFT_CLIP);
    }
