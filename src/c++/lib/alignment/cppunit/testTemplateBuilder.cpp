    ...
    const isaac::alignment::Cluster *cluster)
{
    ...
    f.firstAnchor_.first = 0;
    f.firstAnchor_.second = 10;
    f.firstAnchor_.kUnique_ = true;
    f.lastAnchor_.first = 10;
    f.lastAnchor_.second = 20;
    f.lastAnchor_.kUnique_ = true;
      ...
      , f0_1(getFragmentMetadata(0,107,99,1, true, 1, 1, &cigarBuffer, 2, -12.0, 253, &cluster0))
{
    ...
    int contig = 0;
    BOOST_FOREACH(isaac::reference::ContigAnnotation &contigAnnotation, contigAnnotations)
    {
        // set k-uniqueness to 1 to make sure all candidates are assumed well-anchored in case of CIGAR change
        // Note that with current unit test code, the anchoring is forced by setting the fragment anchors in getFragmentMetadata
        contigAnnotation.resize(contigList.at(contig).size(), std::make_pair<ushort, ushort>(1, 1));
        ++contig;
    }
}
...
void TestTemplateBuilder::testEmptyMatchList()
{
    ...
    std::auto_ptr<TemplateBuilder> templateBuilder(new TemplateBuilder(true, flowcells, 10, 4, false, true, false, false, 8, 2, false, false, false,
                                                                       ELAND_MATCH_SCORE, ELAND_MISMATCH_SCORE, ELAND_GAP_OPEN_SCORE, ELAND_GAP_EXTEND_SCORE,
                                                                       ELAND_MIN_GAP_EXTEND_SCORE, 20000,
                                                                       TemplateBuilder::DODGY_ALIGNMENT_SCORE_UNALIGNED, false));
    ...
    templateBuilder->buildTemplate(contigList, contigAnnotations, restOfGenomeCorrection, readMetadataList, testAdapters, fragments, cluster0, tls);
...
void TestTemplateBuilder::testOrphan()
{
    ...
    using isaac::alignment::FragmentMetadata;
    using isaac::alignment::BandedSmithWaterman;
    TemplateBuilder templateBuilder(true, flowcells, 10, 4, false, true, false, false, 8, 2, false, false, true,
                                    ELAND_MATCH_SCORE, ELAND_MISMATCH_SCORE, ELAND_GAP_OPEN_SCORE, ELAND_GAP_EXTEND_SCORE,
                                    ELAND_MIN_GAP_EXTEND_SCORE, 20000,
                                    TemplateBuilder::DODGY_ALIGNMENT_SCORE_UNALIGNED, false);
    const BamTemplate &bamTemplate = templateBuilder.getBamTemplate();
