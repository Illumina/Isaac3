...
void TestTemplateLengthStatistics::addTemplates(isaac::alignment::TemplateLengthDistribution & tls)
{
    ...
    const isaac::alignment::Cigar cigarBuffer(1, 16);
    std::vector<isaac::alignment::FragmentMetadataList > f(2, isaac::alignment::FragmentMetadataList(1));
    f[0][0].contigId = 0;
    f[0][0].position = 0;
    ...
    f[0][0].cigarOffset = 0;
    f[0][0].cigarLength = 1;
    f[0][0].editDistance = 0;
    f[1][0] = f[0][0];
    f[1][0].reverse = true;
    for (unsigned int i = 1; i < 10000; ++i)
    {
        CPPUNIT_ASSERT_EQUAL(false, tls.appendTemplate(f[0], f[1]));
        ++f[1][0].position;
    }
    ...
    CPPUNIT_ASSERT_EQUAL(true, tls.appendTemplate(f[0], f[1]));
}
