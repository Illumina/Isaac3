    ...
    BinMetadata &binMetadata)
{
    ...
    if (fragment.getTotalLength() !=
        files_.at(binFiles_.at(binMetadata.getIndex())).sputn(reinterpret_cast<const char*>(&fragment), fragment.getTotalLength()))
    {
    ...
    std::size_t file)
{
    if (binMetadata.isEmpty())
    {
        ...
        if (unlink(binMetadata.getPath().c_str()) && ENOENT != errno)
        {
    ...
    {
        // if all neededed files are open at the same time, there is no need to reopen anything.
        if (binFiles_.at(binMetadata.getIndex()) != file)
        {
    ...
    common::ThreadVector &threads)
{
    ISAAC_ASSERT_MSG(std::size_t(std::distance(binsBegin, binsEnd)) <= files_.size(),
                     "Requested more than maximum number of files to be written at the same time:" << std::distance(binsBegin, binsEnd) << " max: " << files_.size());
    ISAAC_THREAD_CERR << "Reopening output files for " << std::distance(binsBegin, binsEnd) << " bins on " << threads.size() << " threads" << std::endl;
    ...
    threads.execute(
        [this, &file, binsBegin, binsEnd](const unsigned threadNumber, const unsigned threadsTotal)
        {
            alignment::BinMetadataList::iterator it = binsBegin + threadNumber;
            while (binsEnd != it)
            {
                reopenBin(*it, std::distance(binsBegin, it));
                it += std::min<std::size_t>(threadsTotal, std::distance(it, binsEnd));
            }
        },
        std::min<std::size_t>(threads.size(), std::distance(binsBegin, binsEnd)));
...
void FragmentBinner::reclaimStorage(
    ...
    const alignment::BinMetadataList::const_iterator binsEnd) noexcept
{
    ...
    BOOST_FOREACH(const BinMetadata &binMetadata, std::make_pair(binsBegin, binsEnd))
    {
        // Not checking for result as if truncate fails we're simply left with longer file which will be read
        // up to binMetadata.getDataSize() anyway.
