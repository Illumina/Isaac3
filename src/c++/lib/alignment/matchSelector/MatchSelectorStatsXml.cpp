    ...
    const TileStats& tileStats) const
{
    ...
    ISAAC_XML_WRITER_ELEMENT_BLOCK(xmlWriter, "Read")
    {
        ...
        ISAAC_XML_WRITER_ELEMENT_BLOCK(xmlWriter, "AlignedFragments")
        {
            xmlWriter.writeElement("AdapterBases", tileStats.adapterBases_);
            if (collectCycleStats_)
            {
                ISAAC_XML_WRITER_ELEMENT_BLOCK(xmlWriter, "MismatchesByCycle")
                {
                    for(uint64_t cycle = read.getFirstCycle(); cycle <= read.getLastCycle(); ++cycle)
                    {
            ...
            xmlWriter.writeElement("Count", tileStats.alignedFragmentCount_);
        }
