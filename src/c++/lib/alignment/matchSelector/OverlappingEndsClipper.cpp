    ...
    BamTemplate &bamTemplate)
{
    ...
    const int64_t overlapLength = left.position + left.getObservedLength() - right.position;
    if (0 >= overlapLength)
    {
        // no overlap
        return;
    ...
    int64_t keepLeft = overlapLength == 1 ? 0 : 1;
    int64_t keepRight = overlapLength == 1 ? overlapLength : overlapLength - 1;
    ...
    if (keepLeft)
    {
        ...
        const reference::Contig::const_iterator reference =
            contigList.at(right.contigId).begin() + right.position;
...
//    else
    if (keepRight)
    {
        ...
        const reference::Contig::const_iterator reference =
            contigList.at(left.contigId).begin() +
