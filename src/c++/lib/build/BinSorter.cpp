...
//            ISAAC_THREAD_CERR << "storeUnaligned: " << offset << "/" << binData.data_.size() << " " << fragment << std::endl;
    ...
    else
    {
        ...
        BOOST_FOREACH(const PackedFragmentBuffer::Index& idx, binData)
        {
            ...
            {
                downgradeAlignmentScores(nodeContigs, nodeAnnotations, idx, binData.data_);
...
{
    reference::Contig::const_iterator currentReference = reference.at(fpos.getContigId()).begin() + fpos.getPosition();
    isaac::reference::ContigAnnotation::const_iterator currentAnnotation = contigAnnotation.begin() + fpos.getPosition();
    ...
    for (const uint32_t *cigarIterator = cigarBegin; cigarIterator != cigarEnd; ++cigarIterator)
    {
        ...
        if (opCode == alignment::Cigar::ALIGN)
        {
            ...
            for (unsigned i = 0; i < length; ++i)
            {
                ...
                {
                    ++matchesInARow;
                    if (isaac::reference::K_UNIQUE_TOO_FAR != currentAnnotation->first &&
                        matchesInARow >= currentAnnotation->first)
                    {
                        return true;
                    }
                }
...
void BinSorter::downgradeAlignmentScores(
    const isaac::reference::ContigLists &contigLists,
    const isaac::reference::ContigAnnotationsList &annotations,
    const PackedFragmentBuffer::Index& idx, PackedFragmentBuffer &data)
{
    ...
    if (fragment.isAligned() && fragment.flags_.realigned_)
    {
        const isaac::reference::ContigAnnotations &contigAnnotations =
            annotations.at(barcodeMetadataList_.at(fragment.barcode_).getReferenceIndex());
        if (!contigAnnotations.empty())
        {
            const isaac::reference::ContigList &reference = contigLists.at(barcodeMetadataList_.at(fragment.barcode_).getReferenceIndex());
