    ...
    PackedFragmentBuffer &dataBuffer)
{
    ...
    if (!index.hasMate() || fragment.flags_.mateUnmapped_)
    {
        ...
        if (index.hasMate())
        {
            ...
            ISAAC_VERIFY_MSG(mate.bamTlen_ == -oldBamTlen,
...
//            ISAAC_THREAD_CERR_DEV_TRACE_CLUSTER_ID(fragment.clusterId_, "leftClippedLeft: " << leftClippedLeft);
    ...
    if(basesLeft > leftClippedLeft + fragment.rightClipped())
    {
        ...
        if (firstUnclippedPos.getPosition() > reference.at(firstUnclippedPos.getContigId()).size())
        {
...
//    ISAAC_THREAD_CERR << "GapRealigner::applyChoice index=" << index << std::endl;
    ...
    if (fragment.leftClipped())
    {
        ...
        ISAAC_VERIFY_MSG(Cigar::SOFT_CLIP == decoded.second, "Original CIGAR is expected to have soft clip at the start " << fragment);
        ISAAC_VERIFY_MSG(fragment.leftClipped() <= decoded.first, "Original CIGAR soft clip at the start is shorter than left-clipped bases" << fragment);
...
//        basesLeft -= fragment.leftClipped();
    ...
    if (fragment.rightClipped())
    {
        ...
        ISAAC_VERIFY_MSG(Cigar::SOFT_CLIP == decoded.second, "Original CIGAR is expected to have soft clip at the end " << fragment);
        ISAAC_VERIFY_MSG(fragment.rightClipped() <= decoded.first, "Original CIGAR soft clip at the end is shorter than right-clipped bases" << fragment);
 ...
 */
int64_t GapRealigner::undoExistingGaps(
    const PackedFragmentBuffer::Index& index,
    const reference::ReferencePosition& pivotPos)
{
    ...
    int64_t position = index.pos_.getPosition();
...
//                ISAAC_THREAD_CERR << " overlapping gaps are not allowed " << overlapPos << "-" << gap << std::endl;
    ...
    if (int64_t(binStartPos.getPosition()) > alignmentPos)
    {
//        ISAAC_THREAD_CERR << " gap places read before bin start" << std::endl;
    ...
    if (int64_t(binEndPos.getPosition()) < alignmentPos)
    {
...
//    ISAAC_THREAD_CERR << "isBetterChoice " << choice << std::endl;
    const bool ret =
        ...
        (choice.cost_ < bestChoice.cost_ || (choice.cost_ == bestChoice.cost_ &&
            (choice.editDistance_ < bestChoice.editDistance_ || (choice.editDistance_ == bestChoice.editDistance_ &&
                choice.totalPriority_ > bestChoice.totalPriority_))));
...
//            const gapRealigner::OverlappingGapsFilter overlappingGapsFilter(gaps);
    ...
    for (GapChoiceBitmask choice = 0; (choice = gapsFilter.next(choice));)
    {
        ...
        int64_t undoneAlignmentPos = undoExistingGaps(index, index.pos_);
        // existing deletion overlaps pivot position like this:
        //A-----C
        //AGATCAG
        //   ^pp
        ISAAC_ASSERT_MSG(undoneAlignmentPos <= int64_t(index.pos_.getPosition()), "undoPivotPos pos " << index.pos_ << " overlapped by an existing deletion " << index);
        ...
        int64_t lastUndoneAlignmentPos = undoneAlignmentPos;
...
//    lastAttemptGaps_.clear();
    ...
    do
    {
        ...
        binEndPos = reference::ReferencePosition(
            binEndPos.getContigId(), std::min<uint64_t>(binEndPos.getPosition(), reference.at(binEndPos.getContigId()).getLength()));
...
//            lastAttemptGaps_ = currentAttemptGaps_;
        ...
        if (bestChoice.choice_)
        {
            if (binEndPos > bestChoice.startPos_)
            {
                ...
                const reference::ReferencePosition contigEndPos(binEndPos.getContigId(), reference.at(binEndPos.getContigId()).size());
