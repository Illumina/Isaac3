...
public:
    ...
    {
        for(PackedFragmentBuffer::iterator it = begin(); end() != it; it += sizeof(isaac::io::FragmentHeader))
        {
            isaac::io::FragmentHeader &header = *(isaac::io::FragmentHeader*)&*it;
            header.flags_.initialized_ = true;
...
void testNoDifferences(std::vector<IndexT> bin, std::vector<IndexT>  expectedUnique)
{
    ...
    std::vector<uint64_t> uniqueFragments;
    ...
    std::vector<uint64_t> expectedUniqueFragments;
