...
void RealignerGaps::finalizeGaps()
{
    ...
    gapGroups_.erase(std::unique(gapGroups_.begin(), gapGroups_.end(), &gapRealigner::Gap::comparePositionAndLength), gapGroups_.end());
...
//    ISAAC_THREAD_CERR_DEV_TRACE("findGaps all gaps: " << gapRealigner::GapsRange(sampleGaps.begin(), sampleGaps.end()));
    ...
    else
    {
        ...
        if (!gapEnds.empty() && !gapStarts.empty())
        {
            ...
            foundGaps.erase(std::unique(foundGaps.begin(), foundGaps.end(), &gapRealigner::Gap::comparePositionAndLength), foundGaps.end());
        }
    }
