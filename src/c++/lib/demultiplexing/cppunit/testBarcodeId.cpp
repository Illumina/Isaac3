...
//    BarcodeId other(4020, 482UL, 1234567UL);
    ...
    // tile
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::TILE_MASK, tile.getTile());
    ...
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::BARCODE_MASK, barcode.getBarcode());
    ...
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::CLUSTER_MASK, cluster.getCluster());
    ...
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::MISMATCHES_MASK, mismatches.getMismatches());
    // none
    ...
    CPPUNIT_ASSERT_EQUAL(0UL, none.getCluster());
    // all
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::TILE_MASK, all.getTile());
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::BARCODE_MASK, all.getBarcode());
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::CLUSTER_MASK, all.getCluster());
    CPPUNIT_ASSERT_EQUAL((uint64_t)BarcodeId::MISMATCHES_MASK, all.getMismatches());
    // OTHER
