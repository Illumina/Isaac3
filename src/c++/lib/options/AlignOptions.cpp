...
AlignOptions::AlignOptions()
    ...
    , expectedCoverage(60) // 30x is current most popular human genome coverage, just make bins a bit smaller than needed to ensure good cpu utilization
    , targetBinSize(0)
    ...
    , verbosity(2)
    , clustersAtATimeMax(2000000) // with 2x100 this is the highest that fits in under 60G ram on 40-thread box.
    ...
    , pessimisticMapQ(false)
{
    ...
    namedOptions_.add_options()
        ...
        ("known-indels"           , bpo::value<bfs::path >(&knownIndelsPath),
                "path to a VCF file containing known indels fore realignment.")
        ...
        ("seed-length"              , bpo::value<unsigned>(&seedLength)->default_value(seedLength),
            "Length of the seed in bases. Only 16 is allowed. Longer seeds reduce sensitivity on noisy data "
            "but improve repeat resolution.")
        ("expected-coverage"         , bpo::value<unsigned>(&expectedCoverage)->default_value(expectedCoverage),
                "Expected coverage is required for iSAAC to estimate the efficient binning of the aligned data.")
        ("target-bin-size"            , bpo::value<uint64_t>(&targetBinSize)->default_value(targetBinSize),
        ...
        ("cleanup-intermediary"  , bpo::value<bool>(&cleanupIntermediary)->default_value(cleanupIntermediary),
                ...
                "still work.")
        ("bcl-tiles-per-chunk"      , bpo::value<unsigned>(&bclTilesPerChunk)->default_value(bclTilesPerChunk),
                "Increase this number when the tiles are too small for the processing to be efficient. In particular, "
                "collecting the template length statistics requires several tens of thousands clusters to work. If tiles are small "
                "and data is heavily multiplexed, there might be not enough clusters in a single tile to collect the tls for a sample")
...
//                "Maximum number of mismatches allowed to accept a gapped alignment when Smith-Waterman is used.")
        ...
        ("memory-limit,m"           , bpo::value<uint64_t>(&memoryLimit)->default_value(memoryLimit),
        ...
        ("buffer-bins"   , bpo::value<bool>(&bufferBins)->default_value(bufferBins),
                "If set, Align will buffer bin data before writing it out. If not set, Align will keep an open "
...
void AlignOptions::parseExecutionTargets()
{
    const static std::vector<std::string> allowedStageStrings =
        boost::assign::list_of("Start")("Align")
    ...
    startFrom =
        ...
        2 == startFromPos ? workflow::AlignWorkflow::AlignDone :
        3 == startFromPos ? workflow::AlignWorkflow::AlignmentReportsDone :
        4 == startFromPos ? workflow::AlignWorkflow::BamDone :
    ...
    stopAt =
        0 == stopAtPos ? workflow::AlignWorkflow::AlignDone :
        1 == stopAtPos ? workflow::AlignWorkflow::AlignDone :
        2 == stopAtPos ? workflow::AlignWorkflow::AlignmentReportsDone :
        3 == stopAtPos ? workflow::AlignWorkflow::BamDone :
        4 == stopAtPos ? workflow::AlignWorkflow::Finish :
                         workflow::AlignWorkflow::Last;
}
...
{
    if (1 < sortedReferenceMetadataList.size())
    {
        BOOST_THROW_EXCEPTION(InvalidOptionException("\n   *** Too many --reference-genome options specified. Only one --reference-genome is allowed in this version. ***\n"));
    }
...
//        BOOST_THROW_EXCEPTION(InvalidOptionException("\n   *** --seed-length other than 16,28,30,32,34,36 and 64 is not supported. ***\n"));
        BOOST_THROW_EXCEPTION(InvalidOptionException("\n   *** --seed-length other than 16 is not supported. ***\n"));
    }
