...
void serialize(xml::XmlReader &reader, SortedReferenceMetadata::AnnotationFile &af, const unsigned int version)
{
    af.k_ = reader("Annotation")["K"];
    const std::string annotationFormat = reader.nextChildElement("Format").readElementText().string();
    if ("8bpb" != annotationFormat && "1bpb" != annotationFormat)
    {
        BOOST_THROW_EXCEPTION(xml::XmlReaderException(std::string("Only 8bpb and 1bpb annotations are supported. got: ") + annotationFormat));
    }

    af.path_ = (reader += "File").readElementText().string();
}


void serialize(xml::XmlReader &reader, SortedReferenceMetadata::AnnotationFiles &annotationFiles, const unsigned int version)
{
    while (reader.nextElementBelowLevel(1) && reader("Annotation"))
    {
        if (reader["Type"] == "k-uniqueness")
        {
            annotationFiles.resize(annotationFiles.size() + 1);
            serialize(reader, annotationFiles.back(), version);
            annotationFiles.back().type_ = SortedReferenceMetadata::AnnotationFile::KUniqueness;
        }
        else if (reader["Type"] == "k-repeatness")
        {
            annotationFiles.resize(annotationFiles.size() + 1);
            serialize(reader, annotationFiles.back(), version);
            annotationFiles.back().type_ = SortedReferenceMetadata::AnnotationFile::KRepeatness;
        }
        else
        {
            BOOST_THROW_EXCEPTION(xml::XmlReaderException(std::string("Only k-uniqueness and k-repeatness annotations are supported")));
        }
    }
...
void serialize<xml::XmlReader>(xml::XmlReader &reader, SortedReferenceMetadata &sortedReferenceMetadata, const unsigned int version)
{
    ...
    // Permutations may not be present
    if (reader && reader.checkName("Annotations"))
    {
        serialize(reader, sortedReferenceMetadata.annotationFiles_, version);
    }
    ...
    const bool makeAbsolutePaths)
{
    ...
    SortedReferenceMetadata ret = loadSortedReferenceXml(is);
    if (makeAbsolutePaths)
    {
        ret.makeAbsolutePaths(xmlPath.parent_path());
    }
    return ret;
}
...
void serialize<xml::XmlWriter>(xml::XmlWriter &writer, const SortedReferenceMetadata &sortedReferenceMetadata, const unsigned int version)
{
    ...
    ISAAC_XML_WRITER_ELEMENT_BLOCK(writer, "SortedReference")
    {
        ...
        if (!sortedReferenceMetadata.annotationFiles_.empty())
        {
            ISAAC_XML_WRITER_ELEMENT_BLOCK(writer, "Annotations")
            {
                BOOST_FOREACH(const SortedReferenceMetadata::AnnotationFile &annotation,
                              sortedReferenceMetadata.annotationFiles_)
                {
                    ISAAC_XML_WRITER_ELEMENT_BLOCK(writer, "Annotation")
                    {
                        writer.writeAttribute(
                            "Type",
                            SortedReferenceMetadata::AnnotationFile::KUniqueness == annotation.type_ ?  "k-uniqueness" : "k-repeatness");
                        writer.writeAttribute("K", annotation.k_);
                        writer.writeElement("Format", "8bpb");
                        writer.writeElement("File", annotation.path_.string());
                    }
                }
            }
        }
